                        ...
                        <div class="card-body">
                            <h4>지역국 리스트</h1>
                                <table class="table table-border">
                                    ...
                                    <tbody>
                                        <c:forEach var="region" items="${list}" varStatus="status">
                                            <tr>
                                                <td>${status.index + 1}</td>
                                                <td><a href="../department/index?regionId=${region.id}">${region.name}</a></td>
                                                <td>
                                                    <form:form method="post" action="../region/delete?id=${region.id}&hqId=${hqId}">
                                                        <a href="../region/update?id=${region.id}"
                                                            class="btn btn-sm btn-success">수정</a>
                                                        <button type="button" class="btn btn-sm btn-danger"
                                                            onclick="deleteRegionConfirm(this)">삭제</button>
                                                    </form:form>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                ...
                                <div class="text-right">
                                    <a class="btn btn-secondary float-left" href="../hq/index">본부 목록</a>
                                    <a href="../region/post?hqId=${hqId}" class="btn btn-primary">추가</a>
                                </div>
                        </div>
                    </div>
                </div>

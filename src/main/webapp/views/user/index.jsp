...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="float-left">관리자 리스트</h4>
                                <select class="form-control float-right" id="admin-row" style="width:100px">
                                    <c:if test="${row==10}">
                                    ...
                                    <tbody>
                                        <c:forEach var="admin" items="${list}">
                                            <tr>
                                                <td>${admin.id}</td>
                                                <td>${admin.email}</td>
                                                <td>${admin.getDepartment().getRegion().getHq().getName()}</td>
                                                <td>${admin.getDepartment().getRegion().getName()}</td>
                                                <td>${admin.getDepartment().getName()}</td>
                                                <td>${admin.getRole().getDescription()}</td>
                                                <td>
                                                    <fmt:parseDate value="${admin.createdDateTime}" pattern="yyyy-MM-dd'T'HH:mm" var="parseDateTime" type="both"/>
                                                    <fmt:formatDate pattern="yyyy.MM.dd HH:mm" value="${parseDateTime}"/>
                                                </td>
                                                <td>
                                                    <form:form method="post" action="../user/delete?id=${admin.id}">
                                                        <a href="#" class="btn btn-sm btn-info" onclick="confirmResetPassword('${admin.id}')">비밀번호 초기화</a>
                                                        <a href="../user/update?id=${admin.id}" class="btn btn-sm btn-success">수정</a>
                                                        <button type="button" class="btn btn-sm btn-danger" onclick="deleteAdminConfirm(this)">삭제</button>
                                                    </form:form>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                ...
                                <div class="text-right">
                                    <a href="../user/post" class="btn btn-primary">추가</a>
                                </div>
                                <div>
                                    <nav aria-label="Page navigation example">
                                        <ul class="pagination justify-content-center">
                                            ...
                                            <c:if test="${pageNo > pageCount}">
                                                <li class="page-item">
                                                    <a class="page-link" href="../user/index?pageNo=${previusPageNo}&row=${row}" aria-label="Previous">
                                                        <span aria-hidden="true">&laquo;</span>
                                                    </a>
                                                </li>        
                                            </c:if>
                                            ...
                                            <c:forEach var="i" begin="${startPageNo}" end="${endPageNo}">
                                                <c:if test="${i == pageNo}">
                                                    <li class="page-item active">
                                                        <a class="page-link " href="../user/index?pageNo=${i}&row=${row}">${i}</a>
                                                    </li>
                                                </c:if>
                                                <c:if test="${i != pageNo}">
                                                    <li class="page-item">
                                            ...
                                            <c:if test="${pageNo != totalPageNo}">
                                                <li class="page-item">
                                                    <a class="page-link" href="../user/index?pageNo=${nextPageNo}&row=${row}" aria-label="Next">
                                                        <span aria-hidden="true">&raquo;</span>
                                                    </a>
                                                </li>
                                            </c:if>
    ...
    <script>
        $("#admin-row").change(function(){
            location.href="/user/index?pageNo=" + '${pageNo}' + "&row=" + this.value;
        });

        function deleteAdminConfirm(btn){
            if(confirm("정말 선택한 관리자를 삭제하시겠습니까?")){
